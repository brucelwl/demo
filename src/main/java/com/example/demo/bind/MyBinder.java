...
public class MyBinder {
	...
	/**
	 * Create a new {@link MyBinder} instance for the specified sources. A
	 * {@link DefaultFormattingConversionService} will be used for all conversion.
	 * @param sources the sources used for binding
	 */
	public MyBinder(ConfigurationPropertySource... sources) {
	 ...
	 * @param sources the sources used for binding
	 */
	public MyBinder(Iterable<ConfigurationPropertySource> sources) {
		this(sources, null, null, null);
	}

	/**
	 * Create a new {@link MyBinder} instance for the specified sources.
	 * @param sources the sources used for binding
	 ...
	 * @see #bind(ConfigurationPropertyName, Bindable, MyBindHandler)
	 */
	 ...
	 * @return the binding result (never {@code null})
	 */
	public <T> BindResult<T> bind(String name, Bindable<T> target, MyBindHandler handler) {
		return bind(ConfigurationPropertyName.of(name), target, handler);
	}

	public <T> BindResult<T> bind(String name, Bindable<T> target,
								  MyBindHandler handler, boolean allowRecursiveBinding) {
		Assert.notNull(name, "Name must not be null");
		Assert.notNull(target, "Target must not be null");
        ConfigurationPropertyName propertyName = ConfigurationPropertyName.of(name);
        handler = (handler != null) ? handler : MyBindHandler.DEFAULT;
		Context context = new Context();
		T bound = bind(propertyName, target, handler, context, allowRecursiveBinding);
	...
	protected final <T> T bind(ConfigurationPropertyName name, Bindable<T> target,
							   MyBindHandler handler, Context context, boolean allowRecursiveBinding) {
	...
	private <T> T handleBindResult(ConfigurationPropertyName name, Bindable<T> target,
								   MyBindHandler handler, Context context, Object result) throws Exception {
		if (result != null) {
			result = handler.onSuccess(name, target, context, result);
			result = context.getConverter().convert(result, target);
		}
		//TODO update by bruce,add method return value
		result = handler.onFinish(name, target, context, result);
		return context.getConverter().convert(result, target);
	}

	private <T> T handleBindError(ConfigurationPropertyName name, Bindable<T> target,
								  MyBindHandler handler, Context context, Exception error) {
		try {
	...
	private <T> Object bindObject(ConfigurationPropertyName name, Bindable<T> target,
		...
		if (property == null && containsNoDescendantOf(context.getSources(), name)) {
			//TODO add by bruce
			if (handler.onNoDescendSkip(name,target,context)){
				return null;
			}
		}
	...
	private <T> Object bindAggregate(ConfigurationPropertyName name, Bindable<T> target,
									 MyBindHandler handler, Context context, AggregateBinder<?> aggregateBinder) {
	...
	private Object bindBean(ConfigurationPropertyName name, Bindable<?> target,
		...
		BeanPropertyBinder propertyBinder = (propertyName, propertyTarget) -> bind(
				name.append(propertyName), propertyTarget, handler, context, allowRecursiveBinding);
	...
	/**
	 * Create a new {@link MyBinder} instance from the specified environment.
	 * @param environment the environment source (must have attached
	 * {@link ConfigurationPropertySources})
	 * @return a {@link MyBinder} instance
	 */
	public static MyBinder get(Environment environment) {
		return new MyBinder(ConfigurationPropertySources.get(environment),
	...
	final class Context implements BindContext {
		...
		Context() {
			this.converter = BindConverter.get(MyBinder.this.conversionService,
					MyBinder.this.propertyEditorInitializer);
		}
		...
		public PlaceholdersResolver getPlaceholdersResolver() {
			return MyBinder.this.placeholdersResolver;
		}
